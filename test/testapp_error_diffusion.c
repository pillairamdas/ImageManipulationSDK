...
int main(int argc, char *argv[])
{
	...
	error_diffuse(input_img, output_img, ERROR_DIFFUSE_FLYD_STEIN);
	if((dump_to_file(output_img->buffer, output_img->size, "OUT_error_diffuse_floyd_stein.raw")) != 0) {
		goto FREE_MEM;
	}
	LOG(OUTPUT, "Output Filename: OUT_error_diffuse_floyd_stein.raw (for Floyd-Steinberg's error diffusion)\n");

	/* Jarvis, Judice, and Ninke (JJN) error diffusion */
	error_diffuse(input_img, output_img, ERROR_DIFFUSE_JJN);
	if((dump_to_file(output_img->buffer, output_img->size, "OUT_error_diffuse_jjn.raw")) != 0) {
		goto FREE_MEM;
	}
	LOG(OUTPUT, "Output Filename: OUT_error_diffuse_jjn.raw (for Jarvis, Judice, and Ninke (JJN) error diffusion)\n");

	/* Stucki error diffusion */
	error_diffuse(input_img, output_img, ERROR_DIFFUSE_STUCKI);
	if((dump_to_file(output_img->buffer, output_img->size, "OUT_error_diffuse_stucki.raw")) != 0) {
		goto FREE_MEM;
	}
	LOG(OUTPUT, "Output Filename: OUT_error_diffuse_stucki.raw (for Stucki error diffusion)\n");

	/* Custom error diffusion */
	error_diffuse(input_img, output_img, ERROR_DIFFUSE_CUSTOM);
	if((dump_to_file(output_img->buffer, output_img->size, "OUT_error_diffuse_custom.raw")) != 0) {
		goto FREE_MEM;
	}
	LOG(OUTPUT, "Output Filename: OUT_error_diffuse_custom.raw (for Custom error diffusion)\n");
